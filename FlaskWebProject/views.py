...
def login():
    ...
    if form.validate_on_submit():
        ...
        if user is None or not user.check_password(form.password.data):
            app.logger.error(f'Failed login attempt by user: {user}')
...
def authorized():
    ...
    if request.args.get('code'):
        ...
        result = _build_msal_app(cache=cache).acquire_token_authorization_code(request.args['code'], scopes=Config.SCOPE, redirect_uri=url_for('authorized', _external=True, _scheme='https'))
        if "error" in result:
            app.logger.error(f'Failed login attempt by {user}')
